#!/bin/sh
# Compile the Cadash Pocket core with Quartus 18.1 in Docker (x86 emulation on ARM Macs).
#
...
FLOW=${1:-compile}
case "$FLOW" in
    map) CMD="quartus_map --read_settings_files=on projects/cadash_pocket.qpf -c cadash_pocket" ;;
    compile) CMD="quartus_sh --flow compile projects/cadash_pocket.qpf" ;;
