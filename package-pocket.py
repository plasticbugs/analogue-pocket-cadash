...
RBF = os.path.join(ROOT, "projects", "output_files", "cadash_pocket.rbf")
PKG = os.path.join(ROOT, "pkg", "pocket")
OUT = os.path.join(ROOT, "release", "pocket")
CORE_ID = "plasticbugs.cadash"
...
# release contains everything needed to produce cadash.rom.
for extra in ("cadash.mra", "README.md", os.path.join("tools", "mra_build.py")):
...
print("build the ROM with:  python3 mra_build.py cadash.mra cadash.zip")
